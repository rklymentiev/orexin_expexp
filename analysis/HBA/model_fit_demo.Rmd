---
title: "Q-Learning model with generated data"
output:
  html_document:
    df_print: paged
---
...
```{r message=FALSE, warning=FALSE}
animal_data <- read_csv("generated_data/agent_data.csv")
true_params <- read_csv("generated_data/true_params.csv")
animal_data <- animal_data %>% 
  rename(animalID = subjID)
  # filter(animalID != "Test2")
...
```{r}
# save(fit_rl, file = "fit_rl_demo.rda")
load(file = "fit_rl_demo.rda")
```
...
for (i in 1:numSubjs) {
  ...
  true_y[i, ] = dat$choice[tmpID, ]  # only for data with a 'choice' column
}
